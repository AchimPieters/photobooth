...
# 2) (aanbevolen) zet een upload-sleutel tegen misbruik
...
- **Upload-sleutel** = dezelfde waarde als `UPLOAD_KEY` (laat leeg als je geen secret zette).

Klaar — na een betaalde sessie verschijnt er een QR-code op het scherm.

## Automatisch opruimen
De Worker verwijdert een foto zodra die ouder is dan `RETENTION_HOURS` (in
`wrangler.toml`). Zet als extra vangnet een **R2 lifecycle-regel** (Cloudflare
dashboard → R2 → bucket → Settings → Object lifecycle) die objecten na 1 dag
verwijdert.
